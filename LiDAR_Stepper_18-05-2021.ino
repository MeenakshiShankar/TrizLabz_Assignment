...
VL53L0X sensor;       //Used to define the sensor object

#define LONG_RANGE
#define HIGH_SPEED

#define pinDirect 3 //Pin for direction of the stepper driver
#define stepPin 4 //Pin for steps of the stepper driver
#define Enable 5 //Pin for enable the stepper driver

//Define the motor controls for the mobile bot
#define m11 A0
#define m12 A1
#define m21 A2
#define m22 A3

int step_time = 1200; //Delay value between steps
...
float maxdist = 5000;           //We set the max distance as per statement to 5m = 5000mm
...
void setup() {
  ...
  pinMode(pinDirect, OUTPUT);
  pinMode(Enable, OUTPUT);
  
  digitalWrite(Enable,LOW);     //Place enable to low so the driver is enabeled
  digitalWrite(pinDirect, HIGH);   //Place pinDirect to HIGH so we spin CW
...
void loop() {
  ...
  delayMicroseconds(step_time);                       //Small delay
  digitalWrite(stepPin, LOW);                     //Make another step
  delayMicroseconds(step_time);                       //Add another delay
